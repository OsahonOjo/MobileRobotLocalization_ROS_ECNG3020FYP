...
@brief This program finds the (real and virtual) intersections between lines derived from the walls of an environment (detected via Hough Transform) for use as landmarks in robot localization.
"""


from __future__ import print_function
from linexline import linexline
...
landmarkPositionPublisher = rospy.Publisher('/HoughLocalization/landmark_position', Point, queue_size=10)  


def cartesianXYToImageXY(cartesianX, cartesianY, imageWidth, imageHeight):
    imageX = int(cartesianX + imageWidth/2)
    imageY = int(imageHeight/2 - cartesianY)
    return imageX, imageY

def imageXYToCartesianXY(imageX, imageY, imageWidth, imageHeight):
    cartesianX = imageX - imageWidth/2
    cartesianY = imageHeight/2 - imageY
...
def plotRangeAndBearingDataOnImage(rangeDataInM, totalSweepAngleInRad, imageWidth, imageHeight, showImage=True):
    ...
    mToCMConversionFactor = 100
    for i in range(len(rangeDataInM)):
        imageX, imageY = cartesianXYToImageXY(x[i]*mToCMConversionFactor, y[i]*mToCMConversionFactor, imageWidth, imageHeight)
    ...
    if showImage == True:
        ...
        cv.waitKey()
    return numPyImageArray

# convention: y = mx + c
def linearEquationBuilder(x1, y1, x2, y2):
    # vertical line
    if x1 == x2:
        return { "m": None, "c": None, "x1": x1, "y1": y1, "x2": x2, "y2": y2 }

    m = (y2-y1)/(x2-x1)
    c = y1 - (m * x1)
    return { "m": m, "c": c, "x1": x1, "y1": y1, "x2": x2, "y2": y2 }

# CONSTRAINT: robot is stationary
# http://docs.ros.org/en/noetic/api/sensor_msgs/html/msg/LaserScan.html
def HoughLocalization(laserScan):
    if 'ranges' not in laserScan or 'angle_max' not in laserScan:
        return -1
    
    ## [Convert Rangefinder Readings to Image]
    laserScanNumPyImage = plotRangeAndBearingDataOnImage(laserScan['ranges'], laserScan['angle_max'], IMAGE_WIDTH, IMAGE_HEIGHT, False)

    ## [edge_detection]
    # https://pyimagesearch.com/2015/04/06/zero-parameter-automatic-canny-edge-detection-with-python-and-opencv/
    sigma = 0.33
    v = np.median(laserScanNumPyImage)
    lowerThreshold = int(max(0, (1.0 - sigma) * v))
    upperThreshold = int(min(255, (1.0 + sigma) * v))
    # cv.Canny(image, threshold1, threshold2[, edges[, apertureSize=3[, L2gradient=False]]]	) -> edges <class 'numpy.ndarray'>
    laserScanEdges = cv.Canny(image=laserScanNumPyImage, threshold1=lowerThreshold, threshold2=upperThreshold)

    # Copy edges to the image that will display the results in BGR color space
    laserScanEdgesBGR = cv.cvtColor(laserScanEdges, cv.COLOR_GRAY2BGR)  # <class 'numpy.ndarray'>

    ## [Probabilistic Line Transform]
    # Probabilistic Line Transform is an optimization of Standard Line Transform
    # For Probabilistic Line Transform, you must decrease the threshold (relative to the Standard Line Transform)
    rhoResolution = 0.5
    ...
    lines = cv.HoughLinesP(image=laserScanEdges, rho=rhoResolution, theta=thetaResolution, threshold=probabilisticThreshold, minLineLength=50, maxLineGap=10)

    ## [Draw Lines]
    print("Hough Transform - Probabilistic Hough Transform")
    if lines is not None:
        for i in range(0, len(lines)):
            l = lines[i][0]     # [x1 y1 x2 y2]
            cv.line(laserScanEdgesBGR, (l[0], l[1]), (l[2], l[3]), (0,255,0), 1, cv.LINE_AA)
    print("type(lines):", type(lines))
    print(lines)

    # # find equation of each line segment using its endpoints
    # lineEquations = []
    # for line in lines:
    #     lineEquations.append(linearEquationBuilder(x1=line[0], y1=line[1], x2=line[2], y2=line[3]))

    # # find equations with similar gradient and y-intercept
    # for i in range(len(lineEquations)-1):
    #     for j in range(i+1,len(lineEquations)):
    #         if lineEquations[i]['m'] == lineEquations[j]['m'] and lineEquations[i]['c'] == lineEquations[j]['c']:
    #             newX1 = lineEquations[i]['x1'] if lineEquations[i]['x1'] < lineEquations[j]['x1'] else lineEquations[j]['x1']
            
    # # form a new line segment using the farthest two points of the smaller line segments

    # [Convert from Cartesian to Image Space]
    linesCartesian = []
    if lines is not None:
        for i in range(len(lines)):
            line = lines[i][0]
            x1, y1 = imageXYToCartesianXY(line[0], line[1], IMAGE_WIDTH, IMAGE_HEIGHT)
            x2, y2 = imageXYToCartesianXY(line[2], line[3], IMAGE_WIDTH, IMAGE_HEIGHT)
            linesCartesian.append([x1, y1, x2, y2])
    print("linesCartesian:", linesCartesian)

    # [Find Points of Intersection]
    intersections = []
    if lines is not None:
        for i in range(len(linesCartesian)-1):
            for j in range(i+1,len(linesCartesian)):
                line1 = linesCartesian[i]
                line2 = linesCartesian[j]
                L1x = [line1[0], line1[2]]
                L1y = [line1[1], line1[3]]
                L2x = [line2[0], line2[2]]
                L2y = [line2[1], line2[3]]
                xi, yi, real = linexline(L1x, L1y, L2x, L2y)
                if xi == float('inf') or yi == float('inf') or math.isnan(xi) or math.isnan(yi):
                    continue    
                if xi < -1*(IMAGE_WIDTH/2) or xi > (IMAGE_WIDTH/2) or yi < -1*(IMAGE_HEIGHT/2) or yi > (IMAGE_HEIGHT/2):
                    continue
                intersections.append({"x": xi, "y": yi, "real": real})
                # cv.circle(img, center, radius, color[, thickness[, lineType[, shift]]]) -> img
                imageX, imageY = cartesianXYToImageXY(xi, yi, IMAGE_WIDTH, IMAGE_HEIGHT)
                cv.circle(img=laserScanEdgesBGR, center=(imageX,imageY), radius=2, color=(0,0,255), thickness=1)
    print("intersections:", intersections)

    # ## [Show Image and Wait]
    # cv.imshow("Source", laserScanNumPyImage)
    # cv.imshow("Detected Lines (Green) and Intersections (Red) - Probabilistic Line Transform", laserScanEdgesBGR)
    # cv.waitKey()
    
    for intersection in intersections:
        publishLandmarkPositions(intersection)

def publishLandmarkPositions(positions):
    point = Point()
    point.x = positions['x']
    point.y = positions['y']
    point.z = positions['real']
...
if __name__ == '__main__':
    try:
        ...
        ranges = morseLandmarkTestWithNoise
        totalSweepAngleInRad = 2*math.pi
        scanTimeInSeconds = 0.1
        laserScan = {
            "angle_min": 0,
            "angle_max": totalSweepAngleInRad,
            "angle_increment": totalSweepAngleInRad / len(ranges),
            "time_increment": scanTimeInSeconds / len(ranges),
            "scan_time": scanTimeInSeconds,
            "range_min": 0.02,
            "range_max": 12,
            "ranges": ranges
        }
        HoughLocalization(laserScan)
