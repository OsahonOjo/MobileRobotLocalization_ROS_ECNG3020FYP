...
# This script continuously receives pose data for robot1 from CoppeliaSim,
...
from final_year_project_py.srv import Robot1Forward40cmControllerReset, Robot1Forward40cmControllerResetResponse
...
command_pub = rospy.Publisher('/robot1_forward40cm_controller/position_command', Int8, queue_size=10)  # pose estimate publisher  
target_pose_pub = rospy.Publisher('/robot1_forward40cm_controller/target_pose', Pose2D, queue_size=10)
...
def handle_reset(request):
	...
	target_pose_reached = False
	target_pose = {'x': 0, 'y': 0}
	return Robot1Forward40cmControllerResetResponse(not target_pose_calculated)
	

def pose_listener():
	rospy.init_node('robot1_forward40cm_controller', anonymous=False)
	rospy.Subscriber('/robot1/pose_estimate', PoseWithCovarianceAndUncertainty, check_pose_reached_callback)		# pose data: meters, rad.
	reset_srv = rospy.Service('robot1_forward40cm_controller_reset', Robot1Forward40cmControllerReset, handle_reset)
